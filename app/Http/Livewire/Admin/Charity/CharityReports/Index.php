...
class Index extends Component
{
    ...
    public function exportExcel()
    {
        ...
        return Excel::download(new ProjectExport($query->get()), 'projects.xlsx');
    }

    private function calculateTotals($items)
    {
        $projectIds = collect($items)->pluck('id')->toArray();
        $ordersProject = OrderDetails::whereIn('item_id', $projectIds)
        ->where('item_type', 'App\Models\CharityProject');

        $this->totalOrderCount = $ordersProject->count();

        $this->totalAmount = $ordersProject->sum('total');
    }

    public function filterTable(){
        $query = CharityProject::with(['trans', 'category', 'category.trans', 'categories', 'orderDetails'])->normal()->orderBy('created_at', 'DESC');
        ...
        if ($this->category_search != "") {
            $query = $query->whereHas('categories', function($q){
                $q->where('category_id', $this->category_search);
            });
            $this->resetPage();
        }
    ...
    public function render()
    {
        ...
        return view('livewire.admin.charity.charity-reports.index', compact('links', 'items', 'categories'));
    }
}
