        ...
        @else
            <div class="payment-methods">
                <div class="img-container">
                    @if ($visaStatus)
                        <button type="button" wire:click="SelectPayment('visa')" id="visa"
                            class="button payment-items  @if ($paymentMethod == 'visa') active @endif">
                            <span>
                    ...
                    @if ($applePayStatus && ($iphone || $safari))
                        ...
                        @if (!config('app.TEST_MODE'))
                            <input type="hidden" id="SHARequestPhrase"
                                value="{{ config('payfort.SHARequestPhrase') }}">
                        @else
                            <input type="hidden" id="SHARequestPhrase" value="96o0CiKlNkSJO7/OJH8ALl$+">
                        @endif
                    @endif
                    @if ($banktransferStatus)
                        <button type="button" wire:click="SelectPayment('bankTransfer')" id="bank"
                            class="button payment-items @if ($paymentMethod == 'bankTransfer') active @endif">
                            <span>
