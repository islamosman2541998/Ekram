<div>
    <div class="CardBox custom-card">
        ...
        <div onclick="window.location.href='{{ route('site.charity-project.show', $project->trans->first()?->slug) }}'"
            class="custom-card-img custom-card-img-bg">
        ...
        <div class="custom-card-body custom-card-body-bg">
            ...
            @if (is_array($donation))
                @switch($donation['type'])
                    @case('unit')
                        @if (!empty($donation['data']) && is_array($donation['data']))
                            <div class="d-flex align-items-center justify-content-between mb-3 custom-card-btn-row">
                                ...
                                @endforeach
                                <input class="form-control custom-card-input" type="number" wire:model.live="unitValueInput"
                                    min="0" placeholder="مبلغ آخر">
                            </div>
                        @endif
                    @break
                    ...
                    @case('fixed')
                        <div class="d-flex align-items-center justify-content-between mb-3 custom-card-btn-row">
                            <label title="{{ @$project['title'] }}" class="amount-btn"
                                style="background-color: {{ @$colors[0] ?? '#3B82F6' }}">
                    ...
                    @case('open')
                        <div class="input-open-container">
                            <input class="form-control open custom-placeholder" type="number" placeholder="@lang('Price')"
                                wire:model="openValue" min="0">
                        </div>
                    @break
            ...
            <div class="d-flex align-items-center justify-content-between mb-3 custom-card-action-row">
                ...
                <a
                    href="{{ route('site.charity-project.show', $project->trans->first()?->slug) }}?amount={{ @$donationAmt }}">
