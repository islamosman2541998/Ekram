        ...
        <div class="modal-dialog modal-xl pt-5" role="document">
            <div class="modal-content mt-5 shadow">
                ...
                <div class="modal-body">
                    @php
                    $Orderdetails = $order?->details?->groupBy('item_type') ?? collect([]);
                    @endphp
                    <table id="" class="table table-sm table-striped">
                        ...
                        @if ($Orderdetails->has('App\Models\Product'))
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>{{ $order?->created_at ? date('H:i:s d-m-Y', strtotime($order->created_at)) : ' ' }}</td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <div class="d-flex justify-content-center bulk-order">
                                    @switch($order?->status)
                                        @case('0')
                                            <a data-hover="@lang('Pending')" class="btn btn-neutral text-warning order-action" data-toggle="tooltip" title="@lang('Pending')">
                                                <i class="bx bx-no-entry"></i>
                                            </a>
                                            @break
                                        @case(1)
                                            <a data-hover="@lang('Confirmed')" class="btn btn-neutral text-success order-action" data-toggle="tooltip" title="@lang('Confirmed')">
                                                <i class='bx bx-check-circle'></i>
                                            </a>
                                            @break
                                        @case(3)
                                            <a data-hover="@lang('Waiting')" class="btn btn-neutral text-info order-action" data-toggle="tooltip" title="@lang('Waiting')">
                                                <i class='bx bx-history'></i>
                                            </a>
                                            @break
                                        @case(4)
                                            <a data-hover="@lang('Canceled')" class="btn btn-neutral text-danger order-action" data-toggle="tooltip" title="@lang('Canceled')">
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('admin.orders.invoices', $order?->id ?? 0) }}" target="_blank">
                    ...
                    {{-- Project ----------------------------------------------------- --}}
                    @if ($Orderdetails->has('App\Models\CharityProject'))
                    ...
                    <div class="table-responsive-lg table-responsive">
                        <table class="table table-striped table-hover table-bordered table-light align-middle">
                            ...
                            <tbody>
                                ...
                                @php
                                $giftCard = $details->gift_details ? json_decode($details->gift_details, true) : [];
                                @endphp
                                <tr>
                                    <td>{{ $details->id ?? ' ' }}</td>
                                    <td>
                        ...
                        <table class="table table-striped table-hover table-bordered table-light align-middle">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $details->id ?? ' ' }}</td>
                                    <td>
                                        <a href="{{ route('admin.eccommerce.products.show', $details->item?->id ?? 0) }}" target="_blank">
                                    ...
                                    <td>
                                        <a href="{{ route('admin.eccommerce.vendors.show', $details->item?->vendor?->id ?? 0) }}" target="_blank">
                                            {{ $details->vendor?->trans?->where('locale', $current_lang)->first()?->title ?? ($details->item?->vendor?->trans?->where('locale', $current_lang)->first()?->title ?? ' ') }}
                                        </a>
                                    </td>
                                    <td>{{ $details->quantity ?? ' ' }}</td>
                                    <td>{{ $details->price ?? ' ' }}</td>
