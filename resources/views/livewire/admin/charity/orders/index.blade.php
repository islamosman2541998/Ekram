                ...
                <form wire:submit.prevent="filters">
                    <div class="row" wire:ignore>
                        <div class="col-md-2 mb-3">
                            <input type="text" value="{{ $search_identifier ?? '' }}"
                                wire:model.lazy="search_identifier" placeholder="{{ trans('orders.identifier') }}"
                                class="form-control">
                        </div>
                        <div class="col-md-2 mb-3">
                            <input type="text" value="{{ $search_name ?? '' }}" wire:model.lazy="search_name"
                                placeholder="@lang('contact_us.search_name')" class="form-control">
                        </div>
                        <div class="col-md-2 mb-3">
                            <input type="text" value="{{ $search_email ?? '' }}" wire:model.lazy="search_email"
                                placeholder="@lang('contact_us.search_email')" class="form-control">
                        </div>
                        <div class="col-md-2 ">
                            <input type="text" value="{{ $search_mobile ?? '' }}" wire:model.lazy="search_mobile"
                                placeholder="@lang('contact_us.search_phone')" class="form-control">
                        </div>
                        ...
                        <div class="col-md-2">
                            <input type="number" step="any" wire:model="search_price_from"
                                placeholder="{{ trans('orders.price_from') }}" class="form-control">
                        </div>
                        <div class="col-md-2">
                            <input type="number" step="any" wire:model="search_price_to"
                                placeholder="{{ trans('orders.price_to') }}" class="form-control">
                        </div>

                        <div class="col-md-2">
                            <input type="date" wire:model="search_created_from"
                                placeholder="{{ trans('orders.created_from') }}" class="form-control">
                        </div>
                        <div class="col-md-2">
                            <input type="date" wire:model="search_created_to"
                                placeholder="{{ trans('orders.created_to') }}" class="form-control">
                        </div>
                        <div class="text-end mt-2">
                            ...
                            <span class="">
                                <button class="btn-danger btn-sm btn text-white"
                                    wire:click="clearSearch">@lang('admin.delete')</button>
                            </span>
                            <span class="">
                                <button class="btn-success btn-sm btn text-white" wire:click="exportExcel()"> Excel
                                </button>
                            </span>
                        </div>
                        <div class="col-md-2 ">
    ...
    @if ($message)
        <div class="alert alert-{{ $msg_type }} alert-dismissible fade show" role="alert">
            <button type="button" class="btn-close" data-bs-dismiss="alert"
                aria-label="Close"></button>{{ $message }}
        </div>
    @endif
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table id="main-datatable" class="table table-striped table-bordered "
                    ...
                    <thead>
                        <tr class="bluck-actions bulk-action-order"
                            @if (empty($mySelected)) style="display: none" @endif scope="row">
                            <td colspan="16">
                                <div class="col-md-12 mt-0 mb-0 text-center">
                                    <button wire:click.prevent="unpublishSelected"
                                        @if (empty($mySelected)) disabled @endif
                                        class="btn btn-warning btn-sm my-2" type="submit">@lang('button.pending')
                                    </button>
                                    <button wire:click.prevent="publishSelected"
                                        @if (empty($mySelected)) disabled @endif
                                        class="btn btn-success btn-sm my-2" type="submit">@lang('button.confirmed')
                                    </button>
                                    <button wire:click.prevent="waitingSelected"
                                        @if (empty($mySelected)) disabled @endif
                                        class="btn btn-info  btn-sm my-2" type="submit">@lang('button.waiting') </button>
                                    <button wire:click.prevent="cancelSelected"
                                        @if (empty($mySelected)) disabled @endif
                                        class="btn btn-primary  btn-sm my-2" type="submit">@lang('button.cancele')
                                    </button>
                                    <button wire:click.prevent="deleteSelected"
                                        @if (empty($mySelected)) disabled @endif
                                        class="btn btn-danger btn-sm my-2" type="submit"> @lang('button.delete')</button>

                                    <div class="btn-group d-inline">
                                        <span class="control-label">@lang('admin.status') : </span>
                                        <select class="btn btn-info btn-xs" style="padding:2px 0px;margin: 0 10px "
                                            wire:model="selectOrderStatus">
                                            <option value="">@lang('admin.status')</option>
                                            @forelse ($orderStatus as $status)
                                                <option value="{{ $status->id }}">
                    ...
                    <tbody>
                        @forelse ($items as $key => $item)
                            <tr class="text-center">
                                <td>
                                    <input type="checkbox" class="checkbox-check" wire:model.ignore="mySelected"
                                        value="{{ $item->id }}"
                                ...
                                <td>
                                    @switch($item->payment_method_id)
                                        @case(3)
                                            <a href="{{ getImage($item->banktransferproof) }}" target="_blank"
                                                class="d-block btn btn-outline-primary btn-sm py-0">
                                        ...
                                        @case(2)
                                            ...
                                            <a type="button" class="d-block btn btn-outline-success btn-sm py-0"
                                                data-bs-toggle="modal" data-bs-target="#payment-details{{ $item->id }}">
                                                {{ $item->payment_method_ar }}<i class="bx bx-credit-card bx-xs"></i>
                                            </a>
                                            <div class="modal fade" id="payment-details{{ $item->id }}" tabindex="-1"
                                                        ...
                                                        @if ($item->payment_proof)
                                                            <div class="table-responsive ">
                                                                <table class="table table-striped table-bordered"
                                                                    dir="ltr">
                                                                    @foreach (json_decode($item->payment_proof, true) as $index => $payment_proof)
                                                                        <tr class="text-end">
                                                                            <td>{{ $index }}</td>
                                                                            <td>
                                                                                @if (is_array($payment_proof))
                                                                                    <pre>{{ json_encode($payment_proof, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                                                                                @else
                                                                                    {{ $payment_proof }}
                                                                                @endif
                                                                            </td>
                                ...
                                <td>
                                    <div class="d-flex justify-content-center bulk-order">
                                        ...
                                        <div wire:ignore.self class="modal fade" id="delete{{ $item->id }}"
                                            tabindex="-1" aria-labelledby="delete{{ $item->id }}"
                                            aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-body">
                                                        <h4 class="swal2-title py-3" id="swal2-title"
                                                            style="display: flex;"> @lang('admin.are_you_sure')</h4>
                                                        <div class="modal-footer p-1">
                                                            ...
                                                            <button type="button"
                                                                ...
                                                                class="btn btn-sm p-1 btn-danger close-modal"
